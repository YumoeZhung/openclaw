...
// duplicate module instances with separate handler variables.
...
export function requestHeartbeatNow(opts?: {
  ...
  console.error(
    `[DBG-WAKE] requestHeartbeatNow: reason=${opts?.reason} handlerSet=${_s.handler !== null} generation=${_s.handlerGeneration} sessionKey=${opts?.sessionKey}`,
  );
  queuePendingWakeReason({
    reason: opts?.reason,
